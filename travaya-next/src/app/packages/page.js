...
export default function Packages() {
    ...
    return (
        <div className="min-h-screen bg-background pb-32 relative overflow-hidden">
            ...
            {/* Cinematic Hero */}
            <div className="relative h-[40vh] sm:h-[80vh] flex items-center justify-center overflow-hidden">
                ...
                <div className="relative z-10 max-w-5xl mx-auto text-center px-4 sm:px-6 pt-20 sm:pt-32 space-y-5 sm:space-y-12">
                    <div className="hidden sm:inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/20 border border-primary/30 text-primary text-[10px] font-black uppercase tracking-[0.3em] backdrop-blur-xl animate-in fade-in slide-in-from-bottom-4 duration-700">
                        <Zap size={12} />
                        <span>Instant Discovery Protocol</span>
                    </div>

                    {/* Clean Luxury Editorial Block */}
                    <div className="relative group/title inline-block py-6 sm:py-12 px-4 sm:px-6">
                        ...
                        <h1 className="relative z-10 text-4xl sm:text-5xl md:text-7xl lg:text-[10rem] font-black tracking-[0.05em] leading-[0.9] animate-in fade-in slide-in-from-top-8 duration-1000 uppercase text-center">
                        ...
                        <div className="hidden sm:flex items-center justify-center gap-4 mt-8 opacity-40 group-hover:opacity-100 transition-opacity">
                    ...
                    <p className="hidden sm:block text-gray-300 max-w-3xl mx-auto text-base md:text-xl font-medium leading-relaxed opacity-90 animate-in fade-in slide-in-from-bottom-12 duration-1200">
            ...
            <div className="max-w-7xl mx-auto px-3 sm:px-6 -mt-16 sm:-mt-24 relative z-30 space-y-6 sm:space-y-12">

                {/* Search & Control Station */}
                <div className="bg-card/40 backdrop-blur-3xl border border-border-custom rounded-2xl md:rounded-[3.5rem] shadow-2xl p-3 sm:p-4 md:p-10 space-y-3 sm:space-y-5 md:space-y-8 animate-in fade-in zoom-in duration-1000">
                    <div className="flex flex-col md:flex-row gap-4 sm:gap-8 items-center">
                        <div className="relative flex-1 group w-full">
                            <Search className="absolute left-4 sm:left-7 top-1/2 -translate-y-1/2 text-primary group-focus-within:scale-110 transition-transform" size={18} />
                            <input
                                type="text"
                                placeholder="Search..."
                                value={searchQuery}
                                ...
                                className="w-full bg-background/50 border border-border-custom pl-10 sm:pl-14 pr-4 sm:pr-6 py-3 sm:py-4 md:py-6 rounded-xl md:rounded-3xl focus:ring-4 focus:ring-primary/5 focus:border-primary outline-none transition-all text-sm md:text-xl font-bold placeholder:text-gray-500 placeholder:uppercase placeholder:tracking-wider placeholder:text-[9px] sm:placeholder:text-sm"
                            />
                        </div>
                        ...
                        <div className="flex items-center gap-2 sm:gap-3 w-full md:w-auto">
                            ...
                            <button
                                ...
                                className="lg:hidden flex items-center gap-1.5 px-3 py-2 bg-card/50 border border-border-custom rounded-xl font-black uppercase tracking-widest text-[9px] hover:border-primary/50 transition-all"
                            >
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3 sm:gap-6 pt-3 sm:pt-6 border-t border-border-custom/50">
                        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                            <div className="flex items-center gap-2 sm:gap-3 px-3 py-1.5 sm:py-2 bg-background/40 border border-border-custom rounded-xl sm:rounded-2xl">
                                <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-primary rounded-full animate-pulse" />
                                <span className="hidden sm:inline text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">Live: <span className="text-foreground">{filteredDestinations.length}</span> Horizons Found</span>
                                <span className="sm:hidden text-[9px] font-black uppercase tracking-[0.2em] text-gray-400"><span className="text-foreground">{filteredDestinations.length}</span> Results</span>
                            </div>
                ...
                {showMobileFilters && (
                    <div className="fixed top-[72px] inset-x-0 bottom-0 z-[100] flex justify-end lg:hidden" onClick={() => setShowMobileFilters(false)}>
                        ...
                        >
                            {/* Sticky header */}
                            <div className="sticky top-0 z-10 bg-card flex items-center justify-between px-6 sm:px-8 py-5 border-b border-b border-border-custom/50 shadow-sm">
                                    ...
                                    <div className="space-y-2">
                                        {CATEGORIES.map(category => (
                                            <button
                                                ...
                                                className={`w-full flex items-center justify-between p-4 rounded-2xl transition-all text-xs font-black uppercase tracking-tighter ${selectedCategory === category.id
                            ...
                            <div className="p-3 space-y-2">
                                {CATEGORIES.map(category => (
                                    <button
                                        ...
                                        className={`w-full flex items-center justify-between p-5 rounded-[2rem] transition-all group relative overflow-hidden ${selectedCategory === category.id
                        ...
                        {loading ? (
                            ...
                            <div className="space-y-16 animate-in fade-in slide-in-from-bottom-12 duration-1000">
                                {viewMode === 'map' ? (
                                    ...
                                    <div className={`${viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-12' : 'flex flex-col gap-6 sm:gap-10'}`}>
                            ...
                            <div className="bg-card/40 backdrop-blur-2xl border border-border-custom rounded-[2rem] md:rounded-[4rem] p-8 sm:p-10 md:p-24 text-center flex flex-col items-center gap-5 sm:gap-10 animate-in zoom-in duration-700">
                                <div className="w-16 sm:w-24 md:w-40 h-16 sm:h-24 md:h-40 bg-primary/10 text-primary rounded-2xl sm:rounded-[2rem] md:rounded-[3rem] flex items-center justify-center relative shadow-2xl">
                                    <Compass size={36} className="animate-[spin_15s_linear_infinite] opacity-50 sm:hidden" />
                                    <Compass size={48} className="animate-[spin_15s_linear_infinite] opacity-50 hidden sm:block md:hidden" />
                                    <Compass size={80} className="animate-[spin_15s_linear_infinite] opacity-50 hidden md:block" />
                                    <div className="absolute inset-0 bg-primary/10 rounded-full blur-3xl animate-pulse" />
                                </div>
                                <div className="space-y-2 sm:space-y-3">
                                    <h3 className="text-xl sm:text-2xl md:text-4xl font-black tracking-tighter uppercase italic">No Trails Detected</h3>
                                    <p className="text-gray-500 max-w-sm mx-auto leading-relaxed font-medium text-sm sm:text-base md:text-lg">
                                ...
                                <button
                                    ...
                                    className="bg-primary hover:bg-black hover:text-primary text-black font-black px-10 sm:px-16 py-4 sm:py-6 rounded-2xl shadow-2xl shadow-primary/20 transition-all transform active:scale-95 uppercase tracking-tighter italic text-base sm:text-xl"
                                >
                            ...
                            <div className="grid grid-cols-1 gap-4">
                                {CATEGORIES.map(category => (
                                    <button
                                        ...
                                        className={`p-6 rounded-[2rem] transition-all text-sm font-black uppercase tracking-[0.2em] relative overflow-hidden ${selectedCategory === category.id
